...
def preprocess(input_file, model_nef_descriptor):
    # Open image file
    image = Image.open(input_file)
    image = image.convert("RGB")
    
    # Get model input dimensions (shape order: BxCxHxW)
    ...
    image = image.resize((model_input_width, model_input_height), Image.BILINEAR)
    data = np.array(image)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="KL720 Kneron Model Zoo Generic Data Inference Example. - YoloX."
    )
    parser.add_argument(
        "-p",
        "--port_id",
    ...
    parser.add_argument(
        "-img",
        "--img_path",
        default=os.path.join(PWD, '../../res/images/auo_test4.jpg'),
        type=str,
    )
    parser.add_argument(
        "-nef",
        ...
        default=os.path.join(
            PWD, '../../res/models/KL720/kn-model-zoo-mmdet_yolox/models_720_auov4.nef'
        ),
        type=str,
    )
    ...
    print("nef_model_path", nef_model_path)

    """
    check device USB speed (Recommend run KL720 at super speed)
    """
    try:
        if kp.UsbSpeed.KP_USB_SPEED_SUPER != get_device_usb_speed_by_port_id(
            usb_port_id=usb_port_id
        ):
            print(
                "\033[91m" + "[Warning] Device is not run at super speed." + "\033[0m"
            )
    except Exception as exception:
        print(
            "Error: check device USB speed fail, port ID = '{}', error msg: [{}]".format(
                usb_port_id, str(exception)
            )
        )
        exit(0)
    ...
    try:
        ...
        print(
            "Error: connect device fail, port ID = '{}', error msg: [{}]".format(
                usb_port_id, str(exception)
            )
        )
        exit(0)
    ...
    try:
        ...
        model_nef_descriptor = kp.core.load_model_from_file(
            device_group=device_group, file_path=nef_model_path
        )
        print(" - Success")
    ...
    """
    nef_radix = (
        model_nef_descriptor.models[0]
        .input_nodes[0]
        .quantization_parameters.v1.quantized_fixed_point_descriptor_list[0]
        .radix
    )  # only support single model NEF
    ...
    data =  preprocess(image_file_path,  model_nef_descriptor=model_nef_descriptor)
    npu_input_buffer = convert_numpy_to_rgba_and_width_align_4(data)
    ...
    for i in range(LOOP_TIME):
        try:
            ...
            generic_raw_result = kp.inference.generic_data_inference_receive(
                device_group=device_group
            )
        except kp.ApiKPException as exception:
    ...
    kwargs = {
        "input_shape": [model_input_width, model_input_height],
        "num_classes": 11,
        "conf_threshold": 0.25,
        "iou_threshold": 0.5,
        "top_k_num": 20,
    ...
    for bbox in det_res:
        ...
        xmin = xmin * (img_width / model_input_width)
        xmax = xmax * (img_width / model_input_width)
        ymin = ymin * (img_height / model_input_height)
        ymax = ymax * (img_height / model_input_height)
        cv2.rectangle(
